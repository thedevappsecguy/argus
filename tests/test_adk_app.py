...
def test_adk_app_has_stage_agents_in_order() -> None:
    ...
    root_agent = adk_app.build_root_agent()
    ...
    assert [agent.name for agent in root_agent.sub_agents] == adk_app.STAGE_NAMES
...
def test_adk_agents_write_expected_state_keys() -> None:
    ...
    root_agent = adk_app.build_root_agent()
    output_keys = {agent.name: agent.output_key for agent in root_agent.sub_agents}
    assert output_keys == {
...
def test_adk_agents_have_only_stage_relevant_tools() -> None:
    ...
    root_agent = adk_app.build_root_agent()
    agents = {agent.name: agent for agent in root_agent.sub_agents}
...
def test_adk_app_does_not_expose_removed_runtime_tools() -> None:
    ...
    root_agent = adk_app.build_root_agent()
    all_tools = set().union(*(_tool_names(agent) for agent in root_agent.sub_agents))
...
def test_downstream_agents_use_ingestion_result_context() -> None:
    ...
    root_agent = adk_app.build_root_agent()
    for agent in root_agent.sub_agents[1:]:
...
def test_element_binding_uses_schema_validated_payload() -> None:
    ...
    root_agent = adk_app.build_root_agent()
    agents = {agent.name: agent for agent in root_agent.sub_agents}
    instruction = agents["element_binding_agent"].instruction
    assert "schema_validation.normalized" in instruction
    assert "{proposed_threats}" not in instruction


def test_adk_app_builds_agents_from_runtime_config() -> None:
    from argus import config
    from argus.agents import adk_app

    runtime_config = config.RuntimeConfig(
        pro_model="gemini-custom-pro",
        flash_model="gemini-custom-flash",
        temperature=0.2,
        api_key="test-key",
    )

    root_agent = adk_app.build_root_agent(runtime_config)
    agents = {agent.name: agent for agent in root_agent.sub_agents}

    assert agents["scenario_enumeration_agent"].model == "gemini-custom-pro"
    assert agents["risk_rating_agent"].model == "gemini-custom-pro"
    assert agents["ingestion_agent"].model == "gemini-custom-flash"
    assert agents["report_agent"].model == "gemini-custom-flash"
    assert agents["report_agent"].generate_content_config.temperature == 0.2
